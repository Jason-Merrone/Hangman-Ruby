...
class GameState
  ...
  def update
    return 1 unless @game_state_map[@game_state_curr].call.zero?
  ...
  def process_game
    puts ''
    puts "#{@word_to_guess.tr(@characters_to_remove,
                              '_')} You have #{@num_tries_remaining} tries remaining. Please enter your guess or type '1' to save and exit"
    input_command
    ...
    @game_state_curr = GameStatesEnum::LOSE unless @num_tries_remaining.positive?
    @game_state_curr = GameStatesEnum::WIN unless @last_user_input.chomp != @word_to_guess && @word_to_guess.tr(
  ...
  def to_json(*_args)
    { 'characters_to_remove' => @characters_to_remove, 'game_state_curr' => 2, 'word_to_guess' => @word_to_guess,
      'num_tries_remaining' => @num_tries_remaining }.to_json
  end
  ...
  def process_load
    ...
    from_json(JSON.parse(File.read(f)))
